...
function guestInfoSections(document: Document): HTMLElement[] {
  const headerRoot = document.querySelector('#borealis-header')?.shadowRoot;
  if (!headerRoot) return [];

  const sections = Array.from(
    headerRoot.querySelectorAll<HTMLElement>('borealis-guest-info-section'),
  );
  const profileMenuRoot = headerRoot.querySelector('borealis-profile-menu')?.shadowRoot;
  if (profileMenuRoot) {
    sections.push(
      ...profileMenuRoot.querySelectorAll<HTMLElement>('borealis-guest-info-section'),
    );
  }
  return sections;
}

function readAtmosBalance(document: Document): number | null {
